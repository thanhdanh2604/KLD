        ...
        if (!empty($filters)) {
            foreach ($filters as $flt) {
                ...
                if (get_option('dce_tokens_filters_whitelist_status') === 'disable') {
                    $used_filters_option = 'dce_tokens_used_filters';
                    $used_filters = get_option($used_filters_option, []);
                    if (!\Elementor\Plugin::$instance->editor->is_edit_mode() && ($_REQUEST['action'] ?? '') !== 'elementor_ajax' && !isset($used_filters[$afilter])) {
                        $used_filters[$afilter] = \true;
                        update_option($used_filters_option, $used_filters);
                    }
